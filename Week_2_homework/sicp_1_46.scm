...
of an extremely general computational strategy known as iterative improvement. 
...
(define (iterative-improve improve good-enough?)
    (define (makeGuess guess) 
        (if (good-enough? guess)
            guess
           (makeGuess (improve guess))
        )
    )
    (lambda (x) (makeGuess x))
)
(define (good-enough? processed-guess) 
    (< processed-guess 0.01)
)
