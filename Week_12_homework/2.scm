...
can be either a 
symbol as usual or 
else a list of two elements. 
In this case, 
the second element is a symbol, the name of the formal parameter. 
The first element is an expression 
whose value is a predicate function that the argument must satisfy. 
That function should return #t if the argument is valid. 

For example, here is a procedure foo that has type-checked parameters num and list:
...
; (define (foo (integer? num) ((lambda (x) (not (null? x))) list)) (print list num))
...
(define (make-frame variables values env)
    ...
    '()
      (if (pair? (car vars))
        (begin 
          (print "pair found")
          (print (car vars))
          (type-check (car vars) (car vals) env)
  ...
  ;did not solve how to hook it up to environments
; (define (test (null? x) (null? y)) (+ x y)) (test 5 6)

(define (type-check var val env)
  (if (pair? var)
    (if (and 
          (procedure? (eval (car var))) 
          (symbol? (cadr var))
        )
        (if (mc-eval (car var) env)
          (print "successful typecheck")
          (error "failed typecheck" var)
        )
        (error "not a valid type argument" var)
    )
        (print "valid symbol")
  )
)
