...
(define (=zero? x) (apply-generic '=zero? x))    ; <-- ADDITION
...
(define (install-scheme-number-package)
  ...
  (put 'equ? '(scheme-number scheme-number)  
       (lambda (x y) (= x y)))
  (put '=zero? '(scheme-number)   ; <-- ADDITION
        (lambda (x) (= 0 x))
...
#|  
 ...
 (scheme-number . 8) 
 STk> (equ? (make-scheme-number 5) (make-scheme-number 3)) ; 
 #f 
 STk> (equ? (make-scheme-number 5) (make-scheme-number 5)) ; 
 #t 
|#
...
(equ? (make-rational 3 5) (make-rational 4 6)) 
